  ...
  return (
    <TeamListViewStyled isValidating={isValidating}>
      ...
      {teamListData === null || teamListData === undefined ? (
        ...
        <TeamListContainer>
          {!teams.length ? <NoneTeamListData>현재 모집중인 팀이 없습니다.</NoneTeamListData> : teams.map((team, index) => <TeamListItem key={index} type="allTeamList" teamData={team} checkNotLast={index + 1 !== teams.length} />)}
