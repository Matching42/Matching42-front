...
const TeamImage = ({ teamMember }) => {
  ...
  const teamMemberLessthan3 = () => teamMember.map((member, index) => <UserImageStyled key={index} size="small" src={`https://cdn.intra.42.fr/users/small_${member}.jpg`} />);
  ...
  return <TeamImageStyled>{teamMemberLen > 3 ? teamMembermorethan3() : teamMemberLessthan3()}</TeamImageStyled>;
};
